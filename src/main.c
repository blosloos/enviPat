                   ...
                   )
{
    ...
    int main(int argc, const char * argv[])
    {
        ...
        unsigned short iso_amount_global = sizeof(isotope_mass) / sizeof(isotope_mass[0]);
        ...
        clock_t start, end;
        double cpu_time_used = 0.0;
        double cpu_time_sum_algo1 = 0.0;
        double cpu_time_sum_algo2 = 0.0;
        double cpu_time_sum_algo3 = 0.0;
        double cpu_time_sum_algo4 = 0.0;
        
        double cpu_time_sum_algo1_all = 0.0;
        double cpu_time_sum_algo2_all = 0.0;
        double cpu_time_sum_algo3_all = 0.0;
        double cpu_time_sum_algo4_all = 0.0;
        
        int a1_faster_a3 = 0;
        int a1_faster_a2 = 0;
        int a1_faster_a4 = 0;
        int a1_3_faster_global = 0;
        int a1_2_faster_global = 0;
        int a1_4_faster_global = 0;
        
        int a1_break_count = 0;
        int a2_break_count = 0;
        int a3_break_count = 0;
        int a4_break_count = 0;
        
        int succesful_calc = 1;
        
        for (int i = iter_start; i < iter_limit; i++) {
            s = sum_formula[i];
            succesful_calc = 1;
            
            double cpu_time_sum_1 = 0.0;
            int m_check_1 = 0;
            double cpu_time_sum_2 = 0.0;
            int m_check_2 = 0;
            double cpu_time_sum_3 = 0.0;
            int m_check_3 = 0;
            double cpu_time_sum_4 = 0.0;
            int m_check_4 = 0;
...
#if SHOW_DETAILS == 1
                printf("\n%d %s",i, s);
                printf(" algo1 __________________________________________");
#endif
				size_t peak_amount = 0;
                for(int j = 0; j < m; j++){
	                start = clock();
                    ...
                    elements = (Element*)calloc(MAX_ELEMENTS, sizeof(Element));
                    
                    if (use_vector) {
                        if ( parse_sum_formula_vector(
                                                      elements,
                                                      s,
                                                      &element_amount,
                                                      &iso_amount,
                    ...
                    int msg_alloc = alloc_peaks(p_l, iso_amount, &m_, &a_, &cc_);
                    if (msg_alloc) {
                        free(elements);
                        succesful_calc = 0;
                        break;
                    ...
                    if(msg || peak_amount < 1){
                        ...
                        cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
                        printf("\nERROR: Algo1 with exit code: %d, %d, sum: %s, elapsed_time: %6.10lf", msg, i, s, cpu_time_used);
	                    break;
	                }else{
                        m_check_1++;
                        if (show_details) {
                            for (int i_ = 0; i_ < element_amount; i_++) {
                                printf("\ncount algo 1 element %s %d", elements[i_].name, elements[i_].all_iso_calc_amount);
                            }
                        }
                        free(elements);
                        free(m_);
                        ...
                        cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
                        cpu_time_sum_1 += cpu_time_used;
	                }
				}
                if (m_check_1) {
#if SHOW_DETAILS == 1
                    printf("\n    peak amount: %ld  elapsed time: %6.10lf, ", peak_amount, cpu_time_sum_1/m_check_1);
#else
                    printf("\n%d, %s, algo1, %ld, %lf, %d, ",i ,s , peak_amount, cpu_time_sum_1/m_check_1, a1_break_count);
#endif
                }
            }
...
#if SHOW_DETAILS == 1
                printf("\n%d %s",i, s);
                printf(" algo2 __________________________________________");
#endif
				size_t peak_amount = 0;
                long c = 0;
                for(int j = 0; j < m; j++){
                    ...
                    if(msg || peak_amount < 1){
                        ...
                        cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
                        printf("\nERROR: Algo2 with exit code: %d, %d, sum: %s, elapsed_time: %6.10lf", msg, i, s, cpu_time_used);
	                    break;
	                }else{
                        c = 0;
	                    for (ptrdiff_t i = 0; i < peak_amount; i++) {
	                    ...
	                    m_check_2++;
                        if (show_details) {
                            printf("\ncount algo 2 %d", elements->all_iso_calc_amount);
                        }
                        free(elements);
                        free(m_);
                        free(a_);
                        ...
                        cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
                        cpu_time_sum_2 += cpu_time_used;
	                }
				}
                if (m_check_2) {
#if SHOW_DETAILS == 1
                    printf("\n    peak amount: %ld  elapsed time: %6.10lf, ", c, cpu_time_sum_2/m_check_2);
#else
                    printf("\n%d, %s, algo2, %ld, %lf, %d, ",i ,s , c, cpu_time_sum_2/m_check_2,a2_break_count);
#endif
                }
            }
            ...
            if (algo_3) {
#if SHOW_DETAILS == 1
                printf("\n%d %s",i, s);
                printf(" algo3 __________________________________________");
#endif
                size_t peak_amount = 0;
                ...
                for(int j = 0; j < m; j++){
	                ...
	                long double a_monoisotopic = 1.0;
	                for (ptrdiff_t  i = 0; i < element_amount; i++) {
                    ...
                    int msg = calc_pattern_algo_3( elements, &peak_amount, t, iso_amount, element_amount, a_monoisotopic, p_l, l_n, rtm, &m_, &a_, &cc_);
                    if(msg || peak_amount < 1){
	                    free(elements);
						free(m_);
						free(a_);
                        ...
                        cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
                        printf("\nERROR: Algo3 with exit code: %d, %d, sum: %s, elapsed_time: %6.10lf", msg, i, s, cpu_time_used);
	                    break;
	                }else {
                        if (show_details) {
                            for (int i_ = 0; i_ < element_amount; i_++) {
                                printf("\ncount algo 3 element %s %d", elements[i_].name, elements[i_].all_iso_calc_amount);
                            }
                        }
                        free(elements);
                        free(m_);
                        ...
                        cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
                        cpu_time_sum_3 += cpu_time_used;
	                }
				}
                if (m_check_3) {
#if SHOW_DETAILS == 1
                    printf("\n    peak amount: %ld  elapsed time: %6.10lf, ", peak_amount, cpu_time_sum_3/m_check_3);
#else
                    printf("\n%d, %s, algo3, %ld, %lf, %d, ",i ,s , peak_amount, cpu_time_sum_3/m_check_3, a3_break_count);
#endif
                }
...
#if SHOW_DETAILS == 1
                printf("\n%d %s",i, s);
                printf(" algo4 __________________________________________");
#endif
                size_t peak_amount = 0;
                long double max_a = 0.0;
...
//                    free(monoisotopic);
                    ...
                    int msg = calc_pattern_algo_4( elements, &peak_amount, t, iso_amount, element_amount, a_monoisotopic, p_l, l_n, rtm, &m_, &a_, &cc_, &max_a);
                    if(msg || peak_amount < 1){
                        free(elements);
                        free(m_);
                        free(a_);
                        ...
                        cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
                        printf("\nERROR: Algo4 with exit code: %d, %d, sum: %s, elapsed_time: %6.10lf", msg, i, s, cpu_time_used);
                        break;
                    }else {
                        if (show_details) {
                            for (int i_ = 0; i_ < element_amount; i_++) {
                                printf("\ncount algo 4 element %s %d", elements[i_].name, elements[i_].all_iso_calc_amount);
                            }
                        }
                        free(elements);
                        free(m_);
                        ...
                        cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;
                        cpu_time_sum_4 += cpu_time_used;
                    }
                }
                if (m_check_4) {
#if SHOW_DETAILS == 1
                    printf("\n    peak amount: %ld  elapsed time: %6.10lf, ", peak_amount, cpu_time_sum_4/m_check_4);
#else
                    printf("\n%d, %s, algo4, %zu, %lf, %d, ",i ,s , peak_amount, cpu_time_sum_4/m_check_4, a4_break_count);
#endif
            ...
            if (show_diffs) {
                if (m_check_1) {
                    cpu_time_sum_algo1_all += cpu_time_sum_1/m_check_1;
                }
                
                if (m_check_2) {
                    cpu_time_sum_algo2_all += cpu_time_sum_2/m_check_2;
                }
                
                if (m_check_3) {
                    cpu_time_sum_algo3_all += cpu_time_sum_3/m_check_3;
                }
                
                if (m_check_4) {
                    cpu_time_sum_algo4_all += cpu_time_sum_4/m_check_4;
                }
                
                if (succesful_calc) {
                    if (m_check_1) {
                        cpu_time_sum_algo1 += cpu_time_sum_1/m_check_1;
                    }
                    
                    if (m_check_2) {
                        cpu_time_sum_algo2 += cpu_time_sum_2/m_check_2;
                    }
                    
                    if (m_check_3) {
                        cpu_time_sum_algo3 += cpu_time_sum_3/m_check_3;
                    }

                    if (m_check_4) {
                        cpu_time_sum_algo4 += cpu_time_sum_4/m_check_4;
                    }
                
                    if (m_check_1 && m_check_3) {
                        a1_3_faster_global++;
                        
                        if (cpu_time_sum_1/m_check_1 <= cpu_time_sum_3/m_check_3) {
                            a1_faster_a3++;
                        }
                        printf("\ndiff t(algo1) - t(algo3): %6.10lf  a1 faster than a3: %d of %d", cpu_time_sum_algo1 - cpu_time_sum_algo3,a1_faster_a3,a1_3_faster_global);
                    }
                    if (m_check_1 && m_check_2) {
                        a1_2_faster_global++;
                        
                        if (cpu_time_sum_1/m_check_1 <= cpu_time_sum_2/m_check_2) {
                            a1_faster_a2++;
                        }
                        
                        printf("\ndiff t(algo1) - t(algo2): %6.10lf  a1 faster than a2: %d of %d", cpu_time_sum_algo1 - cpu_time_sum_algo2,a1_faster_a2,a1_2_faster_global);
                    }
                    
                    if (m_check_1 && m_check_4) {
                        a1_4_faster_global++;
                        
                        if (cpu_time_sum_1/m_check_1 <= cpu_time_sum_4/m_check_4) {
                            a1_faster_a4++;
                        }
                        
                        printf("\ndiff t(algo1) - t(algo4): %6.10lf  a1 faster than a4: %d of %d", cpu_time_sum_algo1 - cpu_time_sum_algo4,a1_faster_a4,a1_4_faster_global);
                    }
                }
            }
        }
        ...
        printf("\nsum algo1: %6.10lf, sum algo2: %6.10lf, sum algo3: %6.10lf, sum algo4: %6.10lf", cpu_time_sum_algo1, cpu_time_sum_algo2, cpu_time_sum_algo3, cpu_time_sum_algo4);
        printf("\nall algo1: %6.10lf, all algo2: %6.10lf, all algo3: %6.10lf, all algo4: %6.10lf", cpu_time_sum_algo1_all, cpu_time_sum_algo2_all, cpu_time_sum_algo3_all, cpu_time_sum_algo4_all);
        return 0;
    }
